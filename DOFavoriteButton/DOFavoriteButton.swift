    ...
    fileprivate func createLayers(image: UIImage!) {
        ...
        for i in 0 ..< 5 {
            ...
            line.path = {
                ...
                let lineFrameCenter = CGPoint(x: lineFrame.midX, y: lineFrame.midY)
                path.move(to: lineFrameCenter)
                let point = CGPoint(x: lineFrame.origin.x + lineFrame.width / 2, y: lineFrame.origin.y)
